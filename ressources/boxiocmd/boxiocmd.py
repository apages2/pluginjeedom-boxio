...
try:
	from lib.boxio_socket import *
except ImportError:
	print "Error: importing module from lib folder"
	sys.exit(1)
	
try:
	from lib.boxio_utils import *
except ImportError:
	print "Error: module lib/boxio_utils not found"
	sys.exit(1)
	
try:
	from lib.boxio_command import *
except ImportError:
	print "Error: module lib/boxio_command not found"
	sys.exit(1)	
...
class config_data:
	def __init__(
		...
		trigger_timeout = 10,
		loglevel = "info",
		logfile = "boxiocmd.log",
		program_path = "",
		sockethost = "",
		socketport = "",
		daemon_pidfile = "boxio.pid",
		log_msg = False,
		log_msgfile = "",
		):
...
def print_version():
	"""
	Print BOXIOCMD version, build and date
	"""
	logger.debug("print_version")
 	print "BOXIOCMD Version: " + __version__
...
def option_listen():
	...
	try:
		serversocket = boxiocmdSocketAdapter(config.sockethost,int(config.socketport))
	...
	if os.path.exists( configFile ):
		...
		if dom:
			...
			try:
				...
				logfile = os.path.join(program_path, "boxiocmd.log")
...
def main():
	...
	# Get directory of the boxiocmd script
	config.program_path = os.path.dirname(os.path.realpath(__file__))

	parser = OptionParser()
	parser.add_option("-d", "--device", action="store", type="string", dest="device", help="The serial device of the USB/CPL, example /dev/ttyACM0")
	parser.add_option("-l", "--listen", action="store_true", dest="listen", help="Listen for messages from USB/CPL")
	parser.add_option("-s", "--sendmsg", action="store", type="string", dest="sendmsg", help="Send one message to USB/CPL")
	parser.add_option("-f", "--boxiostatus", action="store_true", dest="boxiostatus", help="Get USB/CPL device status")
	parser.add_option("-o", "--config", action="store", type="string", dest="config", help="Specify the configuration file")
	parser.add_option("-V", "--version", action="store_true", dest="version", help="Print boxiocmd version information")
	...
	# LOGHANDLER
	if options.debug:
		logger = logger_init(cmdarg.configfile,'boxiocmd', True)
	else:
		logger = logger_init(cmdarg.configfile,'boxiocmd', False)
	
	logger.debug("Python version: %s.%s.%s" % sys.version_info[:3])
	logger.debug("BOXIOCMD Version: " + __version__)
	...
	# ----------------------------------------------------------
	# GET BOXIO STATUS
	if options.boxiostatus:
		cmdarg.rawcmd = boxiocmd.status
		option_send()
